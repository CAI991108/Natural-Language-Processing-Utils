...
# 定义automatic reasoning and tool-use的提示模板
art_prompt = '''
...
问题: {question}
选项: {option}
让我们一步一步地思考，并在适当的地方使用工具。
'''

def use_tool(tool_name, input_data):
    # 模拟工具使用，这里可以调用实际的工具API
    return f"使用工具 {tool_name} 处理 {input_data} 的结果"

def generate_query(data, examples, technique="art"):
    if technique == "art":
        prompt = art_prompt
    else:
...
def prepare_data(input_path, output_path, technique="art"):
...
if __name__ == '__main__':
    ...
    output_path = "./data/art_prepared.jsonl"
    api_key = "sk-"
    base_url = "https://api.siliconflow.cn/v1/chat/completions"
    after_api_path = "./data/art_aftgpt.jsonl"
    wrong_ans_path = "./data/art_wrong_ans.json"
    score_path = "./data/art_score.json"
    technique = "art"
